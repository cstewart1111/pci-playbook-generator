...
// Packages that cannot be bundled (native modules, dynamic loaders, etc.)
const external = [
...
const sharedConfig = {
  ...
  // Make sure CJS-only packages (e.g. express) continue to work in ESM output
  banner,
};
...
async function buildAll() {
  ...
  // Full server bundle — includes app.listen() for Replit / standalone Node.js
  await esbuild({
    ...sharedConfig,
  ...
  // App-only bundle — Express app without listen(), used by the Vercel serverless wrapper (api/index.mjs)
  await esbuild({
    ...sharedConfig,
